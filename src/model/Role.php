    ...
    public function getRole($condition, $withPermissionId = false)
    {
        ...
        if (is_array($condition)) {
            ...
            if (is_numeric($condition) && $condition > 0) {
                $role = Db::name('role')->where('id', $condition)->find();
                if (!empty($role) && $withPermissionId) {
                    $role['permission_ids'] = Db::name('role_permission')->where('role_id', $condition)->column('permission_id');
                }
                return $role;
            }
        }
        $role = Db::name('role')->where($where)->select()->toArray();
        if (!empty($role) && $withPermissionId) {
