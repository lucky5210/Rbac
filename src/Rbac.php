...
namespace mfcad\rbac;


use mfcad\nestedsets\NestedSets;
use mfcad\rbac\model\Permission;
use mfcad\rbac\model\PermissionCategory;
use mfcad\rbac\model\Role;
use mfcad\rbac\model\UserRole;
use think\Db;
use think\db\Query;
use think\db\Where;
use think\Exception;
...
class Rbac
{
     ...
     * @var string
     */
    private $saltToken = 'mfcadet9#$@#GS#$%080asdfaasdg';
    ...
    public function createRole(array $data = [], $permissionIds = '')
    {
        ...
        $model->data($data);
        try{
            $res = $model->saveRole($permissionIds,$data);
            return $res;
        } catch (Exception $e) {
            throw new Exception($e->getMessage());
        }
     ...
     * @throws \think\exception\DbException
     */
    public function generateToken($userId, $timeOut = 10, $prefix = '')
    {
        ...
        Cache::set($freshTOken, $token, $timeOut * 24);
        return [
            'token' => $token,
